...
function Invitation({ token }: { token: string }) {
  ...
  return <main className="public-invite noise" data-testid="public-invitation">
      ...
      <div className="public-hero-grid">
        <div className="public-hero-copy fade-up">
          ...
          <strong>{invite.name}</strong>
           <p>Será uma alegria celebrar 50 anos dessa história com vocês.</p>
    ...
    <footer className="public-footer"><div className="public-footer-names">{coupleLabel}</div><div className="public-footer-note">Com amor, sempre · 50 anos</div></footer>
     ...
     {rsvpOpen && <div className="public-modal-backdrop" role="presentation" onMouseDown={(eventModal) => { if (eventModal.target === eventModal.currentTarget) setRsvpOpen(false); }}><div className="public-modal" role="dialog" aria-modal="true" aria-labelledby="rsvp-title"><div className="public-modal-head"><div><div className="public-section-kicker">Sua resposta</div><h2 id="rsvp-title">Que bom ter você aqui.</h2></div><button className="public-modal-close" onClick={() => setRsvpOpen(false)} aria-label="Fechar confirmação" data-testid="button-close-rsvp"><X size={16} /></button></div><div className="public-modal-body"><div className="mb-5 rounded border border-[#d6c5b2] bg-[#fbf5ed] p-4"><div className="label mb-2">Convite para</div><div className="font-semibold">{invite.name}</div><p className="mt-2 text-sm leading-relaxed text-[#765e6d]">Será uma alegria celebrar 50 anos dessa história com vocês.</p>{hasCompanionFields && companionSlots.length > 0 && <div className="mt-3 space-y-2"><div className="label mb-1">Acompanhantes</div>{companionSlots.map((companion, index) => <div key={`summary-${index}`} className="flex items-center gap-2 text-sm text-[#765e6d]" data-testid={`text-companion-summary-${index}`}><span className="h-1.5 w-1.5 rounded-full bg-[#9b6b40]" />{companion.name ? <span>{companion.name} <span className="text-xs">({companion.relation || 'Acompanhante'})</span></span> : <span>Vaga de acompanhante</span>}</div>)}</div>}</div><div><label className={`public-choice ${choice === 'confirmed' ? 'selected' : ''}`}><input type="radio" name="rsvp-choice" checked={choice === 'confirmed'} onChange={() => setChoice('confirmed')} /><span><strong>{hasCompanionFields ? 'Sim, estaremos presentes' : 'Sim, estarei presente'}</strong><small className="block mt-1 text-[#765e6d]">Mal podemos esperar para celebrar juntos.</small></span></label><label className={`public-choice ${choice === 'declined' ? 'selected' : ''}`}><input type="radio" name="rsvp-choice" checked={choice === 'declined'} onChange={() => setChoice('declined')} /><span><strong>Não poderei comparecer</strong><small className="block mt-1 text-[#765e6d]">Agradeço muito o convite e desejo uma noite linda.</small></span></label></div>{choice === 'confirmed' && hasCompanionFields && <div className="mt-6"><label className="label">Quem estará com você?</label><p className="mb-3 text-xs text-[#765e6d]">Os nomes já definidos aparecem aqui. Preencha apenas as vagas ainda em aberto.</p>{companionSlots.map((companion, index) => <div key={`companion-field-${index}`} className="mb-3"><div className="mb-1 flex items-center justify-between gap-3 text-xs text-[#765e6d]"><span>{companion.name ? `${companion.name} · ${companion.relation || 'Acompanhante'}` : 'Acompanhante ainda não definido'}</span>{!companion.name && <span className="rounded-full bg-[#eadcc9] px-2 py-1 text-[10px]">Vaga disponível</span>}</div>{companion.name ? <div className="rounded border border-[#d6c5b2] bg-[#fbf5ed] px-3 py-2 text-sm text-[#765e6d]" data-testid={`text-defined-companion-${index}`}>{companion.name} <span className="text-xs">({companion.relation || 'Acompanhante'})</span></div> : <input id={`input-companion-name-${index}`} value={names[index] || ''} onChange={(eventInput) => setNames((current) => { const next = [...current]; next[index] = eventInput.target.value; return next; })} className="field" placeholder="Nome do acompanhante" aria-label="Nome do acompanhante" data-testid={`input-participant-${index}`} />}</div>)}</div>}<label className="label mt-5">Uma observação (opcional)<textarea className="field min-h-20" value={note} onChange={(eventInput) => setNote(eventInput.target.value)} data-testid="input-rsvp-note" /></label><button className="public-modal-submit" disabled={respond.isPending} onClick={submitResponse} data-testid="button-submit-rsvp">{respond.isPending ? <Loader2 className="mx-auto animate-spin" size={15} /> : <><Check size={15} className="inline mr-2" /> Confirmar resposta</>}</button></div></div></div>}
    <Flash message={flash} />
  </main>;
}
